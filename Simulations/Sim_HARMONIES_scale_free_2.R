...
# The set of 100 data sets is divided in two for efficiency reasons (only for scale free type and only when
# HARMONIES is applied). Here, the second half of the repetitions.

input_function <- function(X, graph){
  
  
  source("~/Petru/my_harmonies_HARMONIES.R")
  source("~/Petru/my_harmonies_get.network.R")
  source("~/Petru/my_harmonies_est.pcor.R")
  har = my_harmonies_HARMONIES(X, phenotype = rep(0,nrow(X)), beta.stars = 0.2)
...
info = 'e-d_reps_100_zinegbin_samples_500_p_127_cn_100_scale_free_2.RData'
...
time_parallel <- system.time(
  temp <- foreach(i=1:reps) %dopar% {fct(i+50)})
stopCluster(cl)
